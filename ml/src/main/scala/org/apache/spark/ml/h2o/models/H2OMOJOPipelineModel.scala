...
class H2OMOJOPipelineModel(val mojoData: Array[Byte], override val uid: String)
  ...
  def getInputNames(): Seq[String] = {
    val inputMeta = getOrCreateModel().getInputMeta
    (0 until inputMeta.size()).map(idx => inputMeta.getColumnName(idx))
  }
  ...
  def getOutputNames(): Seq[String]  = {
    val outputMeta = getOrCreateModel().getInputMeta
    (0 until outputMeta.size()).map(idx => outputMeta.getColumnType(idx).javatype)
  }
  ...
  /**
    * Return classes for classification problems. If the problem is of regression type, return empty list.
    */
  def getClasses(): Seq[String] = {
    // optimize for the case when Mojo is generated by DAI.
    // Custom generated mojos are not supported
    if(getOrCreateModel().getOutputMeta.size() >= 1){
      getOutputNames()
    }else{
      Seq.empty[String]
    }
